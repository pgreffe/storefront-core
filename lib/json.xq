...
declare function json:nodes-to-json(
    $node as node()*
  ) as xs:string 
{
...
{
   let $attribs := for $attr in $node/@* 
            return if ( fn:matches($attr, '[^0-9.]')) then
               fn:concat(fn:local-name($attr),":'", $attr, "'")
            else fn:concat(fn:local-name($attr),":", $attr, "")
   return if ( fn:count($attribs) = 0 ) then
     ''
   else fn:concat( 'attributes:{',fn:string-join($attribs,','),'},')
};
   ...
   ) as xs:string 
{
   ...
   let $childNames := fn:distinct-values( for $child in $node/* return fn:local-name($child))
    
   let $childJson := 
   for $name in $childNames 
       (: loop over property names gathering up the values in an array if needed :)
     let $valStrings := for $val in $node/*[fn:local-name() = $name]
                             return if ( fn:count($val/*) = 0) then
                      if ( fn:matches(xs:string($val),'[^-]+[^0-9.]')or fn:string-length(xs:string($val))=0) then
                      fn:concat( "'", xs:string($val), "'")
                                      else $val
